            ...
            switch (extension) {
                ...
                case "svg":
                    await file.async('text').then(async function(svgString){ 
                        ...
                        let costumeInfo = getCostumeInfo(getFileNameWithoutExtension(relativePath));
                        let alignment = 9; //9 for custom; 0 for center
                        ...
                        const convertedImage = await convertSvgToPng(svgString, costumeInfo[3], costumeInfo[4]);
                        var image = {
        ...
        if(!sprite.isStage){
            for (let costumeIdx = 0; costumeIdx < sprite.costumes.length; costumeIdx++) {
                ...
                if(assetID == costumeID){
                    let x = (costume.rotationCenterX + 240) / (480 * 4);
                    let y = (costume.rotationCenterY + 180) / (360 * 4);
                    let res = 400 / costume.bitmapResolution;
                    return [x, y, res, costume.rotationCenterX + 240, costume.rotationCenterY] + 180;
                }
            }
        }
    }
